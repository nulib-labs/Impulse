...
enrichment_tasks.py
-------------------
FireWorks tasks for enriching the Northwestern EIS IIIF collection.

Pipeline order (as defined in workflow.yml):
  1.  FetchIIIFCollectionTask   — fetch collection manifest from Northwestern API
  2.  DocumentExtractionTask    — download images + run Marker OCR (existing task)
  3.  GetMetadataTask           — NER + LLM: main_place, key_people
  4.  GeocodeTask               — Nominatim: lat/lon from main_place
  5.  SummaryTask               — LLM: ~125-word summary
  6.  ThemesTask                — LLM: 1-3 controlled-vocabulary themes
  7.  QuotesTask                — LLM: verbatim public comment excerpts
  8.  ContextTask               — LLM: historical context + completed flag
  9.  BuildEnrichedManifestTask — merge all enrichment into IIIF v3 manifest
  10. SaveManifestToMongoTask   — write final manifest to MongoDB

Each task reads docs_path from fw_spec (S3 path to OCR output JSON) and
writes its output to output_path, also passed via fw_spec.

Environment variables required:
  MONGODB_OCR_DEVELOPMENT_CONN_STRING
  AWS credentials (standard boto3 env vars or instance role)
"""
...
# ── MongoDB / FilePad setup (mirrors existing tasks.py) ──────────────────────

mongo_client = MongoClient(os.getenv("MONGODB_OCR_DEVELOPMENT_CONN_STRING"))
...
def call_llm(prompt: str, host: str, model: str = DEFAULT_LLM_MODEL,
...
class AggregateOCRTask(FireTaskBase):
    ...
    def run_task(self, fw_spec: dict) -> FWAction:
        ...
        for acc, blocks in work_blocks.items():
            ...
            logger.info(f"Aggregated {len(blocks)} blocks for {uuid} ({len(text)} chars)")

        save_json(output_path, ocr_output)
        logger.info(f"Wrote {len(ocr_output)} works to pipeline_store key {output_path!r}")
  ...
  "key_people": ["Person One", "Person Two"]
    ...
    def run_task(self, fw_spec: dict) -> FWAction:
        ...
        for doc_id, text in docs.items():
            ...
            people = list(dict.fromkeys(e.text for e in doc.ents if e.label_ == "PERSON"))
            ...
            logger.info(f"  place={results[doc_id]['main_place']}, "
...
{{"PUBLIC_COMMENT": ["'quote one' - Name", "'quote two' - Organization"]}}"""
    ...
    def run_task(self, fw_spec: dict) -> FWAction:
        ...
        for doc_id, text in docs.items():
            ...
            result = self._ask_llm(text, llm_host)
            results[doc_id] = {"doc_id": doc_id, "PUBLIC_COMMENT": result["PUBLIC_COMMENT"]}
...
{{"context": "explanation here", "completed": null}}"""
    ...
    def run_task(self, fw_spec: dict) -> FWAction:
        ...
        for doc_id, text in docs.items():
            ...
            label = {True: "COMPLETED ✓", False: "NOT COMPLETED ✗", None: "UNCLEAR ?"}.get(
                result["completed"], "?"
            )
            logger.info(f"  {label}")
...
class BuildEnrichedManifestTask(FireTaskBase):
    ...
    def run_task(self, fw_spec: dict) -> FWAction:
        ...
        for stub in collection.get("items", []):
            ...
            if thm.get("themes"):
                enriched_metadata.append(self._meta_entry("Themes", ", ".join(thm["themes"])))
            if meta.get("main_place"):
                enriched_metadata.append(self._meta_entry("Main Location", meta["main_place"]))
            if meta.get("coordinates"):
                coords = meta["coordinates"]
                enriched_metadata.append(self._meta_entry(
                    "Coordinates",
                    f"{coords['lat']}, {coords['lon']}"
                ))
            if meta.get("key_people"):
                enriched_metadata.append(self._meta_entry(
                    "Key People", "; ".join(meta["key_people"])
                ))
            if ctx.get("context"):
                enriched_metadata.append(self._meta_entry("Historical Context", ctx["context"]))

            enriched_metadata.append(self._meta_entry(
                "Completed",
                completed_labels.get(ctx.get("completed"), "Unknown")
            ))
                ...
                **stub,
                ...
                "x-impulse:enrichment": {
                    ...
                    "geo": {
                        ...
                        "coordinates": meta.get("coordinates"),  # {lat, lon, display_name}
                    },
        ...
        enriched_collection = {
            **collection,
            ...
            "x-impulse:pipeline": {
                "version":        "2.0.0",
                "model":          llm_model,
                "steps":          ["iiif-fetch", "ocr-marker", "metadata",
                                   "geocode", "summaries", "themes",
                                   "quotes", "context"],
