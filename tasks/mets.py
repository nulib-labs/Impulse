...
from tasks.helpers.funcs import get_s3_text
...
class METSXMLToHathiTrustManifestTask(FireTaskBase):
    ...
    def _convert_mets_to_yml(self, xml_string):
        """
    ...
    def save_to_mongo(self, document, collection=None):
        """Save any Pydantic model to MongoDB."""
        col = collection or self.collection
        coll = helpers._get_db()[col]
        result = coll.update_one(
            {"impulse_identifier": document["impulse_identifier"]},
            {"$set": document},
            upsert=True
        )
    ...
    def run_task(self, fw_spec: dict[str, str]) -> FWAction:
        ...
        input_path = fw_spec["s3_xml_path"]
        output_path = fw_spec["s3_yaml_path"]
        impulse_identifier = fw_spec["impulse_identifier"]
        content = get_s3_text(input_path)

        yaml_content: str = self._convert_mets_to_yml(content)
        s3_path = self.save_to_s3(output_path, yaml_content)
        document = {}
        document["s3_yaml_path"] = output_path
        document["s3_xml_path"] = input_path
        document["impulse_identifier"] = impulse_identifier
        document["raw_xml"] = content
        document["raw_yaml"] = yaml_content
        document["impulse_task_definition"] = str(self._fw_name)
        self.save_to_mongo(document, collection="HathiTrust")
